---
title: "Quickstart"
description: "Get preview-deployer up and running in minutes"
---
...
```yaml
framework: nestjs # or go, laravel, rust, python
database: postgres
health_check_path: /health
app_port: 3000
app_port_env: PORT
app_entrypoint: dist/main.js  # NestJS example; value is framework-dependent
```

See [Configuration Reference](/configuration) for all options. The `app_entrypoint` value depends on your framework (e.g. Go: `server`, Python: `app.main:app`).
...
If you change orchestrator code in the preview-deployer repo (e.g. after pulling changes or editing the TypeScript), run:
...
1. Check the orchestrator logs:
   ...
   # Or: journalctl -u preview-deployer-orchestrator -f
   ```
...
- Read the [Architecture Documentation](/architecture) to understand how it works
