---
title: "Architecture"
description: "System design, components, data flow, and security for preview-deployer"
---
...
- `sync`: Sync orchestrator code to server (build locally, rsync, restart). Use after setup when you change orchestrator code.
...
4. Framework detection (NestJS, Go, Laravel, Rust, Python)
...
- Port allocations are tracked in the deployment store's `portAllocations` map (keyed by deployment id) and are released on cleanup so ports are reused correctly. Allocation excludes host ports currently in use by running Docker containers, so failed deployments whose containers still run do not cause port collisions.
